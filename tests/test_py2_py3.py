    ...
    def test_parse_data_ee_subtypes(self):
        ...
        with self.assertRaises(ValueError):
            ...
            self.assertEqual(parser._parse_data_ee_subtypes(b'\x00\x00\x01\x00\x00\x00AAAA'),
                         ([],u'EE00'))
    def test_QS(self):
        data = b'\x02\x00\x00\x00\x80\x00\x00\x00\x80\x02\x00\x00\x00'
        fmt = 'B2SL'
        result = parser._parse_data_by_format(fmt, data, strict_unsigned=False)
        expected = True, fmt, [2, u'', u'', 2], bytearray()
